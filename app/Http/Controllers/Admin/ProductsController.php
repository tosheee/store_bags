...
class ProductsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $img_wight = $request->input('img_wight');
        $img_height = $request->input('img_height');

        if (isset($img_wight) && isset($img_height)){
            $img_wight = intval($img_wight);
        ...
        {
            $descriptionRequest['upload_main_picture'] = ImagesHelper::resizeImages($request->file('upload_main_picture'), $productId, $request->input('watermark_checked'), $img_wight, $img_height);
        }
        else
        {
        ...
        if($request->hasFile('upload_gallery_pictures') && $request->hasFile('upload_main_picture'))
        {
            ...
            {
                $descriptionRequest['gallery'][$i]['upload_picture'] = ImagesHelper::resizeImages($files_gallery_pic[$i], $productId, $request->input('watermark_checked'), $img_wight, $img_height);
            }
        }
    ...
    public function update(Request $request, $id)
    {
	    ...
	    $water_mark = storage_path('app/public/common_pictures/watermark.png');
            ...
            {
            	$image->resize($img_wight, $img_height)->insert($water_mark, 'bottom-right', 10, 10)->save($path);
            }
            else
            {
            	$image->resize($img_wight, $img_height)->save($path);
            }
